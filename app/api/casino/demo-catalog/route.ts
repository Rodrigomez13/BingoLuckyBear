...
export async function GET() {
  ...
  try {
    ...
    const payload = await response.json() as CasinoDemoGame[] | { games?: CasinoDemoGame[] }
    const entries = Array.isArray(payload) ? payload : payload.games ?? []
    const games = entries.filter((game) => game.symbol && game.name && (game as CasinoDemoGame & { status?: string }).status === 'playable')
