  ...
  return (
    <nav aria-label="Navegación principal" className="lbb-bottom-nav fixed inset-x-0 bottom-0 z-50 md:hidden">
      <div className="mx-auto max-w-md px-3 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2">
        <div className="flex items-stretch justify-between gap-1 rounded-2xl border border-white/10 bg-black/84 px-1.5 py-1.5 shadow-2xl shadow-black/50 backdrop-blur-2xl">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                className={`group flex flex-1 flex-col items-center gap-1 rounded-xl px-1 py-2 text-[9px] font-bold uppercase tracking-wide transition-colors ${
              ...
              >
                <span
                  className={`flex h-9 w-full items-center justify-center rounded-lg transition-all ${
                    active ? 'bg-gradient-to-br from-emerald-300 to-amber-300 shadow-lg shadow-emerald-400/25' : 'bg-transparent'
                  }`}
                >
