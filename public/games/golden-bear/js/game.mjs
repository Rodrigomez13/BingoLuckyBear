import { BETS, INITIAL_CREDITS, MAX_CASCADES, MAX_ROWS, MIN_ROWS, REELS, RETRIGGER_SPINS, SOUND_FILES, STORAGE_KEY, SYMBOLS } from './config.mjs'
...
const math = createGameMath(SYMBOLS, { reels: REELS, minRows: MIN_ROWS, maxRows: MAX_ROWS })
...
function bumpMultiplier(){spinMultiplier++;E.mult.textContent=`×${spinMultiplier}`;E.mult.classList.remove("bump");void E.mult.offsetWidth;E.mult.classList.add("bump");sfx.multiplier()}
...
function captureStickyWilds(currentGrid) {
  ...
  currentGrid.forEach((reel, reelIndex) => reel.forEach((symbol, rowIndex) => {
    if (symbol.wild) stickyWilds.add(`${reelIndex}-${rowIndex}`)
  }))
}
