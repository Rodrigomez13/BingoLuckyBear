...
export function Hero() {
  return (
    <section id="inicio" className="relative overflow-hidden">
      {/* glow */}
      <div
        className="pointer-events-none absolute right-0 top-0 h-[560px] w-[560px] translate-x-1/4 -translate-y-1/4 rounded-full bg-primary/10 blur-3xl"
        aria-hidden="true"
      />
      <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-14 sm:px-6 lg:grid-cols-2 lg:gap-8 lg:py-20 lg:px-8">
        <div>
          <h1 className="font-display text-4xl font-bold uppercase leading-[0.95] tracking-tight text-balance sm:text-5xl lg:text-6xl">
            <span className="block text-foreground">El mejor</span>
            <span className="block text-primary">Truco online</span>
            <span className="block text-foreground">te espera</span>
          </h1>
          ...
          <ul className="mt-8 flex flex-wrap gap-6">
            {features.map((f) => (
              <li key={f.title} className="flex max-w-[9rem] flex-col gap-2">
                <span className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-border bg-card text-primary">
          ...
          <div className="mt-9 flex flex-wrap gap-4">
            <Button
              size="lg"
              className="group bg-primary font-semibold text-primary-foreground hover:bg-primary/90"
            >
              Jugar ahora
        ...
        <div className="relative flex items-center justify-center">
          <div className="relative w-full max-w-md overflow-hidden rounded-3xl border border-primary/20 shadow-2xl lg:max-w-lg">
            <Image
              src="/images/bear-mascot.png"
              alt="Oso mascota de Lucky Bingo Bear con corona sosteniendo cartas de Truco"
              width={620}
              height={620}
              priority
              className="w-full"
